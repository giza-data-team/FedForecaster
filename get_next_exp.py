...
class GetNextExperiment:
    ...
    def check_available_data(self, dataset_name, n_clients, eval_set_size=200):
        # if str(dataset_name) in ["601"]:
        #     return True
        if n_clients == 5 and str(dataset_name) in ['608','605']:
            return True
        elif n_clients == 10 and str(dataset_name) in ['600','602','604','611','612']:
            return True
        elif n_clients == 15 and str(dataset_name) in ['606','607','609','610','611','612']:
            return True
        elif n_clients == 20 and str(dataset_name) in ['601']:
            return True
        try:
            if n_clients == 9:
                print(dataset_name)
            ...
            if n_clients == 10 and dataset_name in ["XLE", "XLF", "XLI", "XLK"]:
                data = pd.read_csv(f"benchmarking_data/{str(dataset_name)}/1.csv")
                return True
