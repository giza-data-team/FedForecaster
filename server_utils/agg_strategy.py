...
# from meta_model.random_model import RandomModel
from .meta_model.random_model import RandomModel
import numpy as np
...
class CustomStrategy(Strategy):
    ...
    def __init__(
        ...
        self.model_path = os.path.join(current_directory, "best_model.json")
        ...
        if aggregator:
            if server_round >= 4:
                ...
                try:
                    if self.loss_flag >= 2:
                        self.loss_flag = 0
                        self.randomModel.set_model()
                        model, hyperParameters = self.randomModel.get_model()
                        data = {'model_name': model, 'model_parameters': hyperParameters}
                        with open(self.model_path, 'w') as file:
                            file.write('\n')  # Ensure each dict is written on a new line
                            json.dump(data, file)
                except:
                    time_now = datetime.now()
                    time_difference = abs(time_now - self.date_from_3)

                    self.save_result.save(num_clients=self.min_fit_clients, dataset_name=self.dataset_name,
                                  test_loss=self.best_loss,
                                  train_loss=self.train_loss, time_taken=time_difference, model=self.best_model,
                                  parameters=self.best_parameters)
                    raise ValueError("Random Search Done")
            else:
    ...
    def aggregate_evaluate(
        ...
        if self.best_loss > loss_aggregated:
            ...
            self.best_model, self.best_parameters = self.randomModel.get_model()
            self.loss_flag = 0
            train_loss = []
            ...
            self.loss_flag = self.loss_flag + 1
        time_now = datetime.now()
        time_difference = abs(time_now - self.date_from_3)

        # Check if the difference is greater than or equal to 15 minutes
        diff_minutes = timedelta(minutes=self.max_time_minutes)
        if time_difference >= diff_minutes or server_round == self.max_round:
            self.save_result.save(num_clients=self.min_fit_clients, dataset_name=self.dataset_name,
                                  test_loss=self.best_loss,
                                  train_loss=self.train_loss, time_taken=time_difference, model=self.best_model,
                                  parameters=self.best_parameters)
            if time_difference >= diff_minutes:
