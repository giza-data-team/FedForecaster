...
class GetClientData:
    ...
    def load_and_process_data(self):
        ...
        try:
            try:
                self.dataset_name = int(self.dataset_name)
                federated_flag = False
                self.data = pd.read_csv(f"benchmarking_data/{self.dataset_name}.csv", parse_dates=['Timestamp'],
                                        index_col='Timestamp').ffill()
            except:
                federated_flag = True
                self.data = pd.read_csv(f"benchmarking_data/{self.dataset_name}/{self.client_id + 1}.csv",
                                        parse_dates=['Timestamp'], index_col='Timestamp').ffill()
...
if __name__ == "__main__":
    data = GetClientData(n_clients=9, client_id=0, dataset_name="XLU").load_and_process_data()
    print(data)
